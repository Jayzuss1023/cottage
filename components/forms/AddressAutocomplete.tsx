...
export function AddressAutocomplete({
  ...
  const parseFeature = (feature: MapboxFeature): AddressResult => {
    ...
    // Patse context for city, state, zipCode
    if (feature.context) {
      for (const ctx of feature.context) {
        if (ctx.id.startsWith("place")) {
          city = ctx.text;
        } else if (ctx.id.startsWith("region")) {
          // Use short_code for state abbrediation ("e.g., "US-CA" -> "CA")
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (!isOpen || setSuggestions.length === 0) return;

    switch (e.key) {
      case "ArrowDown":
        e.preventDefault();
        setHighlightedIndex((prev) =>
          prev < setSuggestions.length - 1 ? prev + 1 : 0,
        );
        break;
      case "ArrowUp":
        e.preventDefault();
        setHighlightedIndex((prev) =>
          prev > 0 ? prev - 1 : setSuggestions.length - 1,
        );
        break;
      case "Enter":
        e.preventDefault();
    ...
    <div ref={containerRef} className={cn("relative", className)}>
      <div className="relative">
        ...
        <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
          {isLoading && (
            <Loader2 className="h-4 w-4 animate-spin text-mued-foreground" />
          )}
