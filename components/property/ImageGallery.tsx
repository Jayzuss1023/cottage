    ...
    <div>
      ...
      >
        ...
        <div className="relative aspect-video w-full rounded-2xl overflow-hidden group shadow-warm">
          ...
          {/* Nav ARROWS : Siblings of Lightbox trigger (no nested buttons) */}
          ...
          {images.length > 1 && (
            <>
              ...
              <Button
                variant="ghost"
                size="icon"
                className="bg-background/80 backdrop-blur-sm shadow-warm absolute right-4 top-1/2 -translate-y-1/2 opacity-0 hover:bg-background group-hover:opacity-100 transition-opacity duration-300 z-10"
                onClick={(e) => {
                  e.stopPropagation();
                  goToNext();
                }}
      ...
      <Dialog open={lightboxOpen} onOpenChange={setLightboxOpen}>
        <DialogContent className="max-w-6xl p-0 bg-black/95 border-none overscroll-contain">
          <DialogTitle className="sr-only">
            Image gallery: {title} - im age {selectedIndex + 1} of{" "}
            {images.length}
          </DialogTitle>
